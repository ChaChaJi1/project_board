package com.stringboot.backboard.service;

import com.stringboot.backboard.model.Board;
import com.stringboot.backboard.repository.BoardRepository;
import com.stringboot.backboard.util.PagingUtil;
...
public class BoardService {
    ...
    public ResponseEntity<Board> getBoard(Integer no) {
        Board board = boardRepository.findById(no)
                .orElseThrow(() -> new ResourceNotFoundException("Not exist Board Data by no L [" + no + "]"));
    ...
    public ResponseEntity<Map<String, Boolean>> deleteBoard(Integer no) {
        //존재하는 데이터인지 조회하여 확인, 없으면 exception 예외 처리한다.
        Board board = boardRepository.findById(no)
                .orElseThrow(() -> new ResourceNotFoundException("Not exist Board Data by no : ["+no+"]"));
